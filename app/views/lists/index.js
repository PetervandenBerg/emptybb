Wrts.views.Lists.Index = Wrts.BaseView.extend({
  templateName: 'lists/index',
  decorator: Wrts.decorators.Lists.Index,

  init: function(options){
    this.viewState = new Backbone.ViewState({
      'showArchivedLists': false
    });
    this.listenTo(Wrts.data.lists, "reset remove toggleArchived", this.render);
    this.render();
    _.defer(function(){ 
      $('[data-toggle="tooltip"]').tooltip();
      if(localStorage.getItem('popState') != 'shown'){
        $('.auto-welcome').trigger("click");
  ...
  events: {
    ...
    'click .tabular__column .body-toggler'    : 'toggleListRow',
  ...
  toggleListRow: function(ev){
    $(ev.target).closest('thead').next().toggleClass('hidden');
    $(ev.target).closest('thead').find('[data-toggle-icon]').toggleClass('fa-minus').toggleClass('fa-plus');
  },
