Wrts.decorators.Profiles.SharedLists = Wrts.BaseDecorator.extend({
  ...
  getData: function() {
    ...
    return {
      ...
      isMyProfile: this.isMyProfile(lists),
  ...
  isMyProfile: function(lists) {
    if (this.collection.models.length > 0) { 
      var firstList = this.collection.models[0],
          sharedUserId = firstList.attributes.user_id,
  ...
  getAccount: function() {
    if (Wrts.data.user) {
      return Wrts.data.user.attributes;
    }
  },
  ...
  getGroupedCollection: function(isArchivedVisible){
    $(this.collection.models).each(function() {
      if (this.attributes.user_id === Wrts.data.user.attributes.id) {
        return this.set('isMyList', true);
      }
    });
